...
function publicSubscription(account: any) {
  ...
  return {
    ...
    planInterval: account.plan_interval ?? null,
    unitAmount: account.unit_amount ?? null,
    currency: account.currency ?? STAYBOOST_CURRENCY,
  ...
  if (body.action === "checkout") {
    ...
    if (account?.stripe_checkout_session_id && !account.stripe_subscription_id) {
      try {
        ...
        if (prior.status === "open" && prior.url) return json({ url: prior.url, reused: true });
      } catch {
        // En gammal eller utgången Checkout-session får inte blockera ett nytt försök.
      }
    }
    ...
    try {
      ...
      return json(
        { error: "checkout_failed", detail: error instanceof Error ? error.message : String(error) },
        502,
      );
    }
  }
