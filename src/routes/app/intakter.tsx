...
function RevenuePage() {
  ...
  const stats = useMemo(() => {
    ...
    const avgStay = relevant.length
      ? relevant.reduce((sum, b) => sum + nightsBetween(b.checkin_date, b.checkout_date), 0) /
        relevant.length
      : 0;
    ...
    <div className="space-y-6">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-end xl:justify-between">
        <div>
          ...
          <p className="mt-2 max-w-2xl text-[13px] leading-relaxed text-[color:var(--ink)]/50">
            Se vad som faktiskt driver bokningsvärde, beläggning och direktförsäljning för{" "}
            {property.name}.
          </p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            onClick={load}
            disabled={loading}
            className="grid h-10 w-10 place-items-center rounded-xl border border-black/[0.07] bg-white text-[color:var(--ink)]/50 hover:text-[color:var(--ink)] disabled:opacity-40"
            title="Uppdatera"
          >
      ...
      {error ? (
        <div className="rounded-2xl border border-red-200 bg-red-50 px-4 py-3 text-[13px] text-red-700">
          Kunde inte läsa all bokningsdata: {error}
        </div>
      ) : null}
      ...
      <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">
        <Metric
          icon={Percent}
          label={`Beläggning · ${windowDays} d`}
          value={fmtPct(stats.occupancy)}
          sub={`${stats.activeUnits} aktiva boenden`}
        />
        <Metric
          icon={Banknote}
        ...
        <Metric
          icon={TrendingUp}
          label="Känd RevPAR"
          value={fmtKr(stats.revpar)}
          sub="Känd intäkt / tillgängliga nätter"
        />
      </div>
      ...
      <div className="grid gap-5 xl:grid-cols-[minmax(0,1.5fr)_minmax(320px,.7fr)]">
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <h2 className="mt-1 font-[Fraunces] text-[22px] font-semibold">
                Beläggning kommande {windowDays} dagar
              </h2>
            </div>
            <span className="rounded-full bg-[#edf3ef] px-3 py-1.5 text-[10px] font-bold text-[#2d684c]">
              {stats.relevant.length} bokningar
            </span>
          </div>
              ...
              <AreaChart data={stats.daily} margin={{ top: 5, right: 5, left: -18, bottom: 0 }}>
                <defs>
                  <linearGradient id="occ" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="5%" stopColor="#2d684c" stopOpacity={0.25} />
                    <stop offset="95%" stopColor="#2d684c" stopOpacity={0.02} />
                  </linearGradient>
                </defs>
                <CartesianGrid vertical={false} stroke="rgba(20,40,30,.07)" />
                <XAxis
                  dataKey="date"
                  tick={{ fontSize: 10, fill: "#7a817c" }}
                  tickLine={false}
                  axisLine={false}
                  minTickGap={30}
                />
                ...
                />
                <Tooltip
                  formatter={(value: number) => [`${Math.round(value)} %`, "Beläggning"]}
                  contentStyle={{
                    borderRadius: 14,
                    border: "1px solid rgba(20,40,30,.1)",
                    fontSize: 12,
                  }}
                />
                <Area
        ...
        <section className="rounded-[24px] border border-black/[0.07] bg-[#173c2b] p-5 text-white shadow-[0_12px_35px_rgba(16,37,27,0.12)] sm:p-6">
          ...
          <h2 className="mt-1 font-[Fraunces] text-[22px] font-semibold text-white">
            Vad sticker ut?
          </h2>
          <div className="mt-5 space-y-3">
            <Pulse
              icon={ArrowUpRight}
              label="Direktbokningsandel"
              value={fmtPct(stats.directShare)}
              hint="Högre andel ger mer kontroll över kundresan."
            />
            <Pulse
              icon={CircleDollarSign}
              label="Tillvalsintäkt"
              value={fmtKr(stats.addonRevenue)}
              hint="Upsell på bokningar med registrerat belopp."
            />
            <Pulse
              icon={CalendarRange}
              label="Genomsnittlig vistelse"
              value={`${stats.avgStay.toFixed(1).replace(".0", "")} nätter`}
              hint="Påverkar både drift och lönsamhet."
            />
            <Pulse
              icon={Banknote}
              label="Väntande betalningar"
              value={fmtKr(stats.pendingValue)}
              hint="Pengar som ännu inte är avstämda."
            />
          </div>
        </section>
      </div>
      ...
      <div className="grid gap-5 lg:grid-cols-2">
        <section className="rounded-[24px] border border-black/[0.07] bg-white p-5 sm:p-6">
          ...
          <h2 className="mt-1 font-[Fraunces] text-[21px] font-semibold">
            Var kommer bokningarna ifrån?
          </h2>
          <div className="mt-5 space-y-3">
                ...
                <div key={row.source}>
                  <div className="flex items-center justify-between text-[12px]">
                    ...
                    <span className="font-bold">
                      {row.count} · {fmtPct(share)}
                    </span>
                  </div>
                  <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-[#edf0eb]">
                    <div
                      className="h-full rounded-full bg-[#2d684c]"
        ...
        <section className="rounded-[24px] border border-black/[0.07] bg-white p-5 sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <h2 className="mt-1 font-[Fraunces] text-[21px] font-semibold">
                En viktig sak om siffrorna
              </h2>
            </div>
          </div>
          <p className="mt-4 text-[12px] leading-relaxed text-[color:var(--ink)]/55">
            Beläggningen räknas på alla bekräftade bokningar. ADR, RevPAR och intäkt bygger däremot
            bara på bokningar där ett belopp finns registrerat i StayBoost. Externa bokningar utan
            belopp gör därför intäktssiffrorna försiktiga — aldrig konstgjort höga.
          </p>
        </section>
      </div>
    </div>
...
function Metric({
  ...
  return (
    <div className="rounded-[22px] border border-black/[0.07] bg-white p-4 shadow-[0_5px_18px_rgba(25,40,31,0.035)] sm:p-5">
      <div className="flex items-center gap-2 text-[color:var(--ink)]/38">
        <Icon size={15} />
        <span className="text-[9px] font-bold uppercase tracking-[0.14em]">{label}</span>
      </div>
      <p className="mt-3 font-[Fraunces] text-[25px] font-semibold leading-none sm:text-[29px]">
        {value}
      </p>
      <p className="mt-2 text-[10px] leading-relaxed text-[color:var(--ink)]/38">{sub}</p>
    </div>
  );
}
...
function Pulse({
  ...
  return (
    <div className="rounded-2xl border border-white/10 bg-white/[0.06] p-4">
      ...
      <p className="mt-2 font-[Fraunces] text-[24px] font-semibold text-white">{value}</p>
