...
describe("canonical event names", () => {
  ...
  it("exposes honesty primary CTA and keeps kom_igang as secondary", () => {
    expect(ANALYTICS_CTAS.OPPN_A_PRODUKTDEMON).toBe("oppn_a_produktdemon");
    expect(ANALYTICS_CTAS.DEMO).toBe("demo");
    expect(ANALYTICS_CTAS.KOM_IGANG).toBe("kom_igang");
  });
});
...
describe("trackCtaClicked", () => {
  it("sends CTA Clicked with honesty primary props", () => {
  ...
  it("sends demo depth props for /produkten cards", () => {
    const { calls } = mockSink();
    trackCtaClicked({
  ...
  it("still accepts secondary kom_igang without inventing a new event name", () => {
    const { calls } = mockSink();
    trackCtaClicked({
      surface: ANALYTICS_SURFACES.LANDING,
    ...
    expect(calls[0]).toEqual({
      name: "CTA Clicked",
      props: { surface: "landing", cta: "kom_igang", location: "header" },
    });
  });
  ...
  it("drops unknown plan values and PII-shaped values", () => {
    ...
    ).toEqual({ surface: "landing", cta: "oppn_a_produktdemon", location: "hero" });
    ...
    ).toEqual({ surface: "landing", cta: "oppn_a_produktdemon" });
  });
});
...
describe("PII guards", () => {
  ...
  it("flags email and phone values", () => {
    ...
    expect(looksLikePiiValue("oppn_a_produktdemon")).toBe(false);
