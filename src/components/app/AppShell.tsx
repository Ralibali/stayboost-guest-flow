            ...
            {NAV.slice(0, 7).map((item) => {
              ...
              return (
                <Link
                  key={item.to}
                  to={item.to}
                  className={`flex shrink-0 items-center gap-1.5 rounded-full px-3 py-2 text-[11px] font-semibold transition ${
                    active ? "bg-[#173c2b] text-white" : "bg-[#f0f2ee] text-[color:var(--ink)]/60"
                  }`}
                >
                  <item.icon size={13} />
                  {item.label}
