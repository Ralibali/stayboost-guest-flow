...
const managedStatuses = new Set(["trialing", "active", "past_due", "unpaid", "paused"]);
...
export function BillingPanel({ mobile = false }: { mobile?: boolean }) {
  ...
  const [busy, setBusy] = useState<"checkout-month" | "checkout-year" | "portal" | null>(null);
  ...
  const run = async (action: "checkout" | "portal", interval?: PlanInterval) => {
    if (!supabase) return;
    const busyKey = action === "portal" ? "portal" : interval === "year" ? "checkout-year" : "checkout-month";
    setBusy(busyKey);
    setError(null);
    const { data, error: invokeError } = await supabase.functions.invoke("saas-billing", {
  ...
  const managed = managedStatuses.has(subscription?.status ?? "");
      ...
      <div className="mx-3 mb-3 rounded-xl border border-black/[0.06] bg-[#f5f6f3] px-3 py-2.5 text-[11px]">
        <div className="flex items-center justify-between gap-3">
          ...
          {state?.enabled && managed ? (
            ...
            >
              {busy ? <Loader2 size={13} className="animate-spin" /> : "Hantera"}
            </button>
          ) : null}
        </div>
        {state?.enabled && !managed ? (
          <div className="mt-2 grid grid-cols-2 gap-2">
            ...
            >
              {busy === "checkout-month" ? <Loader2 size={13} className="mx-auto animate-spin" /> : "449 kr/mån"}
            </button>
            <button
              type="button"
              disabled={Boolean(busy)}
            ...
            >
              {busy === "checkout-year" ? <Loader2 size={13} className="mx-auto animate-spin" /> : "4 490 kr/år"}
            </button>
          </div>
        ) : null}
        {!state?.enabled ? (
          <p className="mt-2 text-[10px] text-[color:var(--ink)]/45">Stripe-abonnemang förbereds.</p>
        ) : null}
      ...
      <div className="flex items-start gap-2.5">
        ...
        <div className="min-w-0 flex-1">
          ...
          {state?.enabled && managed ? (
            ...
            >
              {busy === "portal" ? <Loader2 size={13} className="animate-spin" /> : "Hantera abonnemang"}
            </button>
          ) : null}
          {state?.enabled && !managed ? (
            <div className="mt-2 grid gap-1.5">
              ...
              >
                {busy === "checkout-month" ? <Loader2 size={13} className="animate-spin" /> : "Aktivera · 449 kr/mån"}
              </button>
              <button
                type="button"
                disabled={Boolean(busy)}
              ...
              >
                {busy === "checkout-year" ? <Loader2 size={13} className="animate-spin" /> : "Årsvis · 4 490 kr (2 mån gratis)"}
              </button>
            </div>
          ) : null}
          {!state?.enabled ? (
